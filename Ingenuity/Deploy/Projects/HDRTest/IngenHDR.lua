...
function BeginHDR()
	...
	toneMappingTarget = CreateSurface(1, 1, true, "4x16f");
	logLumTargets = {};
	for i = 1,5 do
	...
	glareDef = glareDefs[11]; --[12]GLT_CINECAM_VERTICALSLIT --[5]GLT_FILTER_CROSSSCREEN
...
function ShadeTonemapping(sourceSurface,deltaTime)
...
function ShadeHDR(sourceSurface,deltaTime)
	if shadersLoaded then	
		BeginTimestamp("tonemapping",true,true);
		ShadeTonemapping(sourceSurface,deltaTime);
