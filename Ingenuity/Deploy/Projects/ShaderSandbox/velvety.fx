...
shadedVertexOutput velvetVS(appdata IN,
    ...
    float3 Nn = normalize(mul(IN.Normal,WorldITXf).xyz); // Normalized Normal
    float4 Po = float4(IN.Position.xyz,1); // Out Position
    OUT.HPosition = mul(Po,WvpXf); 
    float3 Pw = mul(Po,WorldXf).xyz; // World Position
    float3 Ln = normalize(LampPos - Pw); // Lamp Normal
    float ldn = dot(Ln,Nn); // Lamp Dot Normal
    float diffComp = max(0,ldn); 
    float3 diffContrib = diffComp * SurfaceColor;
    float subLamb = smoothstep(-RollOff,1.0,ldn) - smoothstep(0.0,1.0,ldn); // Sub-Lambertian
