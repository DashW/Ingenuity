...
function matrix:rotation( x, y, z )
	-- get matrix table
	local mtx = {}
	-- build new matrix
	for i = 1,4 do
	...
	local tmp_ch = math.cos(y);
	local tmp_sh = math.sin(y);
	local tmp_cp = math.cos(x);
	local tmp_sp = math.sin(x);
	local tmp_cb = math.cos(z);
	local tmp_sb = math.sin(z);
	
	mtx[1][1] = (tmp_ch * tmp_cb) + (tmp_sh * tmp_sp * tmp_sb);
	mtx[1][2] = tmp_sb * tmp_cp;
	mtx[1][3] = (-tmp_sh * tmp_cb) + (tmp_ch * tmp_sp * tmp_sb);
	mtx[2][1] = (-tmp_ch * tmp_sb) + (tmp_sh * tmp_sp * tmp_cb);
	mtx[2][2] = tmp_cb * tmp_cp;
	mtx[2][3] = (tmp_sb * tmp_sh) + (tmp_ch * tmp_sp * tmp_cb);
	mtx[3][1] = tmp_sh * tmp_cp;
	mtx[3][2] = -tmp_sp;
	mtx[3][3] = tmp_ch * tmp_cp;
	mtx[4][4] = 1;
