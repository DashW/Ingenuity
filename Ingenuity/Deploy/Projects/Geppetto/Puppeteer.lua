...
function Begin()
	local appPathArg, puppeteerArg = GetCommandLineArgs();
	
	if puppeteerArg and puppeteerArg == "puppeteer" then
		puppeteer = true;
	end
	...
	performanceSurface = CreateSurface(1.0,1.0,GetMainWindow());
...
function Draw()
	...
	if puppeteer then
		...
		if performanceSurface then
			...
			if drawCar then
				DrawComplexModel(carModel,performanceCamera,light,performanceSurface,nil,revealEffect);
			end
		end
		
		if PROFILING then EndTimestamp("DrawPerformance",false,true) end
		
		-- Draw the performance surface 
		
		if PROFILING then BeginTimestamp("DrawViewports",false,true) end
		...
		if PROFILING then EndTimestamp("DrawViewports",false,true) end
	...
	if PROFILING then
		...
		drawTimeText = string.format("CTRL: %2.2fms, SHDW: %2.2fms, PRFM: %2.2fms, VIEW: %2.2fms",
			...
			GetTimestampData("DrawViewports",false) * 1000);
