...
tp_obj tp_has(TP,tp_obj self, tp_obj k) {
    ...
    if (type == TP_DICT) {
        ...
        const char *p = strstr(TP_CSTR(self),TP_CSTR(k));
...
tp_obj tp_hasattr(TP)
{
	tp_obj scope = TP_OBJ();
	tp_obj key = TP_TYPE(TP_STRING);
	return tp_has(tp,scope,key);
}
...
tp_obj tp_get(TP,tp_obj self, tp_obj k) {
    ...
    if (type == TP_DICT) {
		bool error = false;
		r = _tp_dict_get(tp,self.dict.val,k,error);
		if(error) { tp_raise(tp_None,"%s: KeyError: %s\n","tp_get",TP_CSTR(k)); }
        return r;
    } else if (type == TP_LIST) {
        if (k.type == TP_NUMBER) {
            int l = (int) tp_len(tp,self).number.val;
            int n = (int) k.number.val;
            n = (n<0?l+n:n);
...
void tp_set(TP,tp_obj self, tp_obj k, tp_obj v) {
    ...
    if (type == TP_DICT) {
        ...
        if (k.type == TP_NUMBER) {
            _tp_list_set(tp,self.list.val,(int) k.number.val,v,"tp_set");
            return;
        } else if (k.type == TP_NONE) {
...
tp_obj tp_mul(TP,tp_obj a, tp_obj b) {
    if (a.type == TP_NUMBER && a.type == b.type) {
        ...
        int al = a.string.len; int n = (int) b.number.val;
