...
tp_obj tp_range(TP) {
    ...
    switch (tp->params.list.val->len) {
        ...
        case 2:
        case 3: a = (int) TP_NUM(); b = (int) TP_NUM(); c = (int) TP_DEFAULT(tp_number(1)).number.val; break;
        default: return r;
    }
    if (c != 0) {
...
tp_obj tp_float(TP) {
    tp_obj v = TP_OBJ();
    int ord = (int) TP_DEFAULT(tp_number(0)).number.val;
    int type = v.type;
