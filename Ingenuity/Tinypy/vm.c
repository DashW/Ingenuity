...
void _tp_call(TP,tp_obj *dest, tp_obj fnc, tp_obj params) {
    ...
    if (fnc.type == TP_FNC && !(fnc.fnc.ftype&1)) { // C-defined
...
tp_obj tp_import(TP, char const *fname, char const *name, void *codes) {
	...
	if (!codes) { // file is already compiled
...
void tp_builtins(TP) {
	...
	{"ord",tp_ord}, {"merge",tp_merge}, {"globals",tp_globals}, 
	{"hasattr",tp_hasattr},
	{0,0},
    };
...
tp_obj tp_compile(TP, tp_obj text, tp_obj fname) {
	//tp_obj params = tp_params_v(tp,3,text,fname,mode);
 //   return tp_call(tp,"BUILTINS","compile", params);
...
tp_obj tp_eval(TP, char *text, tp_obj globals) {
 //   tp_obj code = tp_compile(tp,tp_string(text),tp_string("<string>"),tp_string("exec"));
	//if(code.type != TP_NONE) { return tp_exec(tp,code,globals); }
	//tp_raise(tp_None, "Compile Error!");
    tp_obj code = tp_compile(tp,tp_string(text),tp_string("<eval>"));
	if(code.type == TP_NONE) { tp_raise(tp_None,"tp_eval: Syntax Error: %s",text); }
