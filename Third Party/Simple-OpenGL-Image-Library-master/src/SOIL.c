...
int query_NPOT_capability( void )
{
	...
	if( has_NPOT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		for(i = 0; i < n; i++) {
			char const* extStr = (char const*)glGetStringi(GL_EXTENSIONS, i);
			if(strcmp(extStr, "GL_ARB_texture_non_power_of_two") == 0 ||
				strcmp(extStr, "GL_OES_texture_npot") == 0)
			{
...
int query_tex_rectangle_capability( void )
{
	...
	if( has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		for(i = 0; i < n; i++) {
			char const* extStr = (char const*) glGetStringi(GL_EXTENSIONS, i);
			if(strcmp(extStr, "GL_ARB_texture_rectangle") == 0 ||
               strcmp(extStr, "GL_EXT_texture_rectangle") == 0 ||
               strcmp(extStr, "GL_NV_texture_rectangle") == 0)
			{
...
int query_cubemap_capability( void )
{
	...
	if( has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		has_cubemap_capability = true;
#else
		GLint n, i;
		glGetIntegerv(GL_NUM_EXTENSIONS, &n);
		for(i = 0; i < n; i++) {
			char const* extStr = (char const*)glGetStringi(GL_EXTENSIONS, i);
			if(strcmp(extStr, "GL_ARB_texture_cube_map") == 0 ||
				strcmp(extStr, "GL_EXT_texture_cube_map") == 0)
			{
