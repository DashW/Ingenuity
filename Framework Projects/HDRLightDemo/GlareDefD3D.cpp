...
static STARDEF s_aLibStarDef[NUM_BASESTARLIBTYPES] =
{
    ...
    {   L"CrossFilter",    4,      3,      1.0f,   0.95f,  0.0f,     true,   },  // STLT_CROSS
    {   L"snowCross",      6,      3,      1.0f,   0.96f,  _Rad( 20.0f ),    true,   },  // STLT_SNOWCROSS
    {   L"Vertical",       2,      3,      1.0f,   0.96f,  00.0f,    false,  },  // STLT_VERTICAL
};
...
static GLAREDEF s_aLibGlareDef[NUM_GLARELIBTYPES] =
{
    ...
    {   L"Camera",                         1.5f,   1.2f,   1.0f,   0.00f,  1.0f,   STLT_CROSS,
        00.0f,    0.5f,   0.25f,  0.90f,  1.0f,  },   // GLT_CAMERA
    {   L"Natural Bloom",                  1.5f,   1.2f,   0.0f,   0.00f,  0.0f,   STLT_DISABLE,
        00.0f,    0.0f,   0.40f,  0.85f,  0.5f,  },   // GLT_NATURAL
    {   L"Cheap Lens Camera",              1.25f,  2.0f,   1.5f,   0.05f,  2.0f,   STLT_CROSS,
        00.0f,    0.5f,   0.18f,  0.95f,  1.0f,  },   // GLT_CHEAPLENS
    /*  
      {   TEXT("Afterimage"),                     1.5f,   1.2f,   0.5f,   0.00f,  0.7f,   STLT_CROSS,
      _Rad(00.0f),    0.5f,   0.1f,   0.98f,  2.0f,  },   // GLT_AFTERIMAGE
     */
    {   L"Cross Screen Filter",            1.0f,   2.0f,   1.7f,   0.00f,  1.5f,   STLT_CROSSFILTER,
        _Rad( 25.0f ),    0.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_FILTER_CROSSSCREEN
    {   L"Spectral Cross Filter",          1.0f,   2.0f,   1.7f,   0.00f,  1.8f,   STLT_CROSSFILTER,
        _Rad( 70.0f ),    1.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_FILTER_CROSSSCREEN_SPECTRAL
    {   L"Snow Cross Filter",              1.0f,   2.0f,   1.7f,   0.00f,  1.5f,   STLT_SNOWCROSS,
        _Rad( 10.0f ),    0.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_FILTER_SNOWCROSS
    {   L"Spectral Snow Cross",            1.0f,   2.0f,   1.7f,   0.00f,  1.8f,   STLT_SNOWCROSS,
        _Rad( 40.0f ),    1.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_FILTER_SNOWCROSS_SPECTRAL
    {   L"Sunny Cross Filter",             1.0f,   2.0f,   1.7f,   0.00f,  1.5f,   STLT_SUNNYCROSS,
        _Rad( 00.0f ),    0.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_FILTER_SUNNYCROSS
    {   L"Spectral Sunny Cross",           1.0f,   2.0f,   1.7f,   0.00f,  1.8f,   STLT_SUNNYCROSS,
        _Rad( 45.0f ),    1.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_FILTER_SUNNYCROSS_SPECTRAL
    {   L"Cine Camera Vertical Slits",     1.0f,   2.0f,   1.5f,   0.00f,  1.0f,   STLT_VERTICAL,
        _Rad( 90.0f ),    0.5f,   0.20f,  0.93f,  1.0f,  },   // GLT_CINECAM_VERTICALSLIT
    {   L"Cine Camera Horizontal Slits",   1.0f,   2.0f,   1.5f,   0.00f,  1.0f,   STLT_VERTICAL,
...
glm::vec4 CStarDef::ms_avChromaticAberrationColor[];
...
bool CStarDef::Initialize( const wchar_t* szStarName,
                              int nStarLines,
                              int nPasses,
                              float fSampleLength,
                              float fAttenuation,
                              ...
                              bool bRotation )
{
    ...
    float fInc = _Rad( 360.0f / ( float )m_nStarLines );
    for( int i = 0; i < m_nStarLines; i ++ )
    {
...
//  Sunny cross filter
bool CStarDef::Initialize_SunnyCrossFilter( const wchar_t* szStarName,
                                               float fSampleLength,
                                               float fAttenuation,
                                               ...
                                               float fInclination )
{
    ...
    {
        m_pStarLine[i].fSampleLength = fSampleLength;
        m_pStarLine[i].fInclination = fInc * ( float )i + 0.0f;
...
bool CStarDef::InitializeStaticStarLibs()
{
    ...
    glm::vec4 avColor[8] =
    {
		...
		glm::vec4(0.5f, 0.5f, 0.5f, 0.0f), // w
		glm::vec4(0.8f, 0.3f, 0.3f, 0.0f),
		glm::vec4(1.0f, 0.2f, 0.2f, 0.0f), // r
		glm::vec4(0.5f, 0.2f, 0.6f, 0.0f),
		glm::vec4(0.2f, 0.2f, 1.0f, 0.0f), // b
		glm::vec4(0.2f, 0.3f, 0.7f, 0.0f),
		glm::vec4(0.2f, 0.6f, 0.2f, 0.0f), // g
		glm::vec4(0.3f, 0.5f, 0.3f, 0.0f),
    };

	memcpy(ms_avChromaticAberrationColor, avColor, sizeof(glm::vec4) * 8);
    /*
...
bool CStarDef::DeleteStaticStarLibs()
{
	...
	if(ms_pStarLib) delete[] ms_pStarLib;
	ms_pStarLib = 0;
    return true;
}
...
bool CGlareDef::Initialize( const wchar_t* szStarName,
...
{
    // Delete all libraries
    if( ms_pGlareLib ) delete[] ms_pGlareLib;
	ms_pGlareLib = 0;
    return true;
}
